...
async function invokeAnkiConnect(action, params = {}) {
  try {
    let response = await axios_default.post("http://127.0.0.1:8765", {
      action,
      version: 6,
      params
    });
...
function getBlobNameFromUrl(url2) {
  let urlParts = url2.split("/").filter(Boolean);
  let blobName = urlParts.at(-1);
  return blobName;
}
function createIPAFieldValue(pieces) {
  if (pieces.length > 0) {
...
async function main() {
  ...
  let jsonPath = import_path.default.resolve(process.cwd(), args[0]);
  if (!import_fs.default.existsSync(jsonPath)) {
    console.error(`File not found: ${jsonPath}`);
    process.exit(1);
  }
  let userData = JSON.parse(import_fs.default.readFileSync(jsonPath, "utf-8"));
  ...
  try {
    ...
    if (!decks.includes(deckName)) {
      ...
      await invokeAnkiConnect("createModel", {
        ...
        css: import_fs.default.readFileSync(import_path.default.join(__dirname, "card.css"), "utf-8"),
      ...
      let fontPath = import_path.default.join(process.cwd(), "public", "fonts", "Roboto.woff2");
  ...
  let existingNotesMap = /* @__PURE__ */ new Map();
